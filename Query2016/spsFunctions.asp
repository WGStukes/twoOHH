...
'*f2:----------------------------------------------------------------------------------------------------------'
Function errorLog(loc, docNo, eType)
	dim x
	dim fs
	dim f
		x=loc
	set fs=Server.CreateObject("Scripting.FileSystemObject")
	set f=fs.OpenTextFile(Server.MapPath("log/errorLog.txt"),8,true)
		f.WriteLine(Date & " //// " & Time & " //// " & x & " //// " & docNo & "//// " & eType)
		f.Close
	set f=Nothing
	set fs=Nothing
	IF eType <> "Document Not Found" THEN
		sendError loc,docNo,eType
	END IF
End Function
...
on error resume next
	...
	str = str & "<label for='document_no'>New Query:&nbsp;&nbsp;</label>"
...
'*f12:---------------------------------------------------------------------------------------------------------'
Function getContactData(orgId,addId)
on error resume next
...
'END IF
IF wkData.wkActEnd = "<I>Unavailable</I>" THEN
	str = str & "</td><th>Due: </th><td> " & wkData.wkend
ELSE
	str = str & "</td><th>Completion Date: </th><td> " & wkData.wkActEnd
END IF
...
Function logUse(file)
dim x
dim fs
dim f
x=loc
Dim cip
SET cip= Request.ServerVariables("HTTP_X_FORWARDED_FOR")
IF cip = "" THEN
	SET cip = Request.ServerVariables("REMOTE_ADDR")
END IF

set fs=Server.CreateObject("Scripting.FileSystemObject")
set f=fs.OpenTextFile(Server.MapPath(file),8,true)
f.WriteLine(date & "////" & time & "////" & cip & "////" & doc_no)
f.Close
...
'*f49:---------------------------------------------------------------------------------------------------------'
function sendError(database, doc, error)
Dim body

body=""
body = body & date & " //// " & time & " //// " & database & " //// "& error & " //// " & doc & vbCrLf & vbCrLf
body = body & "Contracts BSO"
body = body & "Probable Database Connection Error, please create a ticket and forward to the technical team"

Set myMail=CreateObject("CDO.Message")
myMail.Subject="ASP Tools"
myMail.From="suzy.sturgeon@navy.mil"
myMail.To="M_CHLK_NW-2.0-HELPDESK_UD@navy.mil"
myMail.TextBody=body
myMail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/sendusing"     ) = 2
myMail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/smtpserver"    ) = "SDNI-CLIN127Relay.nadsuswe.nads.navy.mil"
...
FOR ii = 0 to 3
	objConn.Open connections(ii)
		IF objConn.State <> 1 THEN
			objID = -1
								errorLog sites(ii), doc_no, "Database Connection Failure"
		ELSE
		...
		END IF
				IF objID <> -1 THEN
					...
					site_id = sites(ii)
						IF sites(ii) = "CHLK" THEN
							file = "log/chlUseLog.txt"
						ELSEIF sites(ii) = "ORL" THEN
							file = "log/orlUseLog.txt"
						ELSEIF sites(ii) = "PAX" THEN
							file = "log/paxUseLog.txt"
						END IF
					logUse(file)
						objConn.Close
					EXIT FOR
...
IF connString = -1 THEN
						errorLog "All Sites Searched", doc_no, "Document Not Found"
END IF
...
Function setSite(conn, docNo)
	...
	ELSE
				...
				CASE ELSE
					this.site_id = -1
					errorLog "log/errorLog.txt", docNo, "Document Not Found"
...
Function printFooter()
	...
	str = str & "		<a href='https://wdcontracts.navair.navy.mil/contracts/internal/default.asp'>Contracts Home </a>"
	str = str & "	|	<a href='https://wdcontracts.navair.navy.mil/contracts/tools/Query2016/Query.asp'>Query</a>"
	str = str & "	| 	<a href='https://wdcontracts.navair.navy.mil/contracts/tools/Query2016/helpTicket.asp'>Report an Issue</a>"
	str = str & "	|	<a target='_blank' href='https://wdcontracts.navair.navy.mil/contracts/tools/Query2016/Query User Manual.pdf'>User Guide </a>"
	str = str & "	| 	<a href='#' onclick='return showAbout()'> About</a>"
	str = str & "</div><!-- footer -->"
...
str = str & "			<input id='document_no' maxlength='24' name='doc_no' size='30' value=''/>"
str = str & "			<input type='submit' onclick='return ckChar()' value='Search'/><br/>"
str = str & "			<span id='exTxt'>"
str = str & "				<small>"
str = str & "					Example: N68936-08-D-0016, N68936-08-D-0016-P00030, N68936-08-D-0016%"
str = str & "				</small>"
str = str & "			</span>"
str = str & "		</form>"
str = str & "	</div>"
...
'f53:---------------------------------------------------------------------------------------------------------'
Function printBanner(hdr, doc)
Dim str
str = ""
str = str & "		<div class='banner'></div><!--banner-->"
str = str & "					<div class='navy'>"
str = str & "						<a href = 'https://wdcontracts.navair.navy.mil/contracts/internal/default.asp'>Contracts Home</a>"
str = str & "						<p><i>	"
						Select Case site_id
							Case "CHLK"
str = str & 									"<i>China Lake Database Connected</i>"
							Case "PAX"
str = str & 									"<i>PAX Database Connected</i>"
							Case "ORL"
str = str & 									"<i>Orlando Database Connected</i>"
							Case Else
str = str & 									"<i>CHLK Database Connected</i>"
						End Select
str = str & "						</i></p>"
str = str & "					</div><!--navy-->"
